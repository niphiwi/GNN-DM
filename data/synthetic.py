...
from utils.transforms import Add2DMask, Apply2DMask, RandomSubgraphTransform
...
class SyntheticDataset(InMemoryDataset):
    ...
    Args:
        ...
        sliding_step (int): The sliding step for the sequence.
        num_nodes (int): The number of nodes in the graph.
        p_sensor (float): The percentage of sensor nodes.
        transform (torch_geometric.transforms.Compose): The transformations to apply to the data.
        temporary (bool): If True, the dataset is loaded without the graph transformation.
    """
    def __init__(self, root: str, type: Literal["train", "valid", "test"], 
                 seq_len: int=10, sliding_step: int=4, num_nodes: int=750, p_sensor=None, transform=None, temporary=False):
        ...
        if transform is None:
            if not temporary:
                self.transform = T.Compose([
                    RandomSubgraphTransform(self.num_nodes),
                    T.Distance(),
                    Add2DMask(seq_len=10, percentage=self.p_sensor),
                    Apply2DMask(),
                ])
            else:
                self.transform = T.Compose([
    ...
    def process(self):
        data = torch.load(self.raw_paths[0], weights_only=True)
    ...
    def get(self, idx):
        if not self.temporary:
            ...
            data = super().get(idx)
            data = T.RadiusGraph(r=0.5, loop=True, max_num_neighbors=300)(data)
...
class SyntheticDataModule(pl.LightningDataModule):
    ...
    """
    def __init__(self, seq_len: int=10, batch_size: int=32, num_workers: int=0, shuffle: bool=False, num_nodes=750, p_sensor=0.7,
                 transform=None):
        super().__init__()
        self.seq_len = seq_len
    ...
    def setup(self, stage: str):
        # Train dataset
        self.train_dataset = SyntheticDataset(root="data/30x25", type="train", num_nodes=self.num_nodes, p_sensor=self.p_sensor, transform=self.transform)  
        # Val     
        self.val_dataset = SyntheticDataset(root="data/30x25", type="valid", num_nodes=750, p_sensor=0.05, transform=self.transform)    # Validate on fixed params for comparability  
        # Test
        self.test_dataset = SyntheticDataset(root="data/30x25", type="test", num_nodes=self.num_nodes, p_sensor=self.p_sensor, transform=self.transform)  
    
    def train_dataloader(self):
        return DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=self.shuffle, num_workers=self.num_workers, persistent_workers=False)
    
    def val_dataloader(self):
        return DataLoader(self.val_dataset, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers, persistent_workers=False)
